...
void ScrollText::setFont(QFont font)
{
	_font = font;
}
...
void ScrollText::paintEvent(QPaintEvent*)
{
	...
	if (scrollEnabled)
	{
		...
		while (x < width())
		{
			pb.drawStaticText(QPointF(x, (height() - wholeTextSize.height()- _font.pixelSize()) / 2) + QPoint(2, 2), staticText);
...
void ScrollText::resizeEvent(QResizeEvent*)
{
	...
	if (width() > 64)
	{
		...
		alphaChannel.fill(_foregroundColor.rgb());
